...
public class DoctorDashboard extends JPanel {
    ...
    // ── Stat labels ───────────────────────────────────────────────────────────
    private JLabel lblPatientsSeen    = makeStatValue("...");
    private JLabel lblPrescriptions   = makeStatValue("...");
    private JLabel lblLabOrders       = makeStatValue("...");
    private JLabel lblEncountersToday = makeStatValue("...");
    ...
    private JPanel buildHeader() {
        ...
        JLabel sub = new JLabel("Welcome, Dr. " + LoginScreen.loggedInEmail +
    ...
    private JPanel buildStatCards() {
        ...
        row.add(buildStatCard("🏥", "Patients Seen",       lblPatientsSeen,    SKY,   SKY_SOFT));
        row.add(buildStatCard("💊", "Prescriptions Issued", lblPrescriptions,   LILAC, LILAC_SOFT));
        row.add(buildStatCard("🧪", "Lab Orders Created",   lblLabOrders,       MINT,  MINT_SOFT));
        row.add(buildStatCard("📅", "Encounters Today",     lblEncountersToday, PEACH, PEACH_SOFT));
    ...
    private JPanel buildRecentActivity() {
        ...
        JLabel title = new JLabel("📋  Recent Clinical Activity");
        title.setFont(F_SECTION);
        title.setForeground(TEXT_DARK);

        String[] cols = {"Type", "Patient / Details", "Date", "Status"};
        ...
        JTable table = new JTable(activityModel) {
            @Override public Component prepareRenderer(TableCellRenderer r, int row, int col) {
                ...
                c.setBackground(row % 2 == 0 ? CARD_WHITE : new Color(245, 248, 255));
        ...
        table.getTableHeader().setForeground(new Color(80, 120, 180));
        table.getTableHeader().setFont(new Font("Segoe UI Emoji", Font.BOLD, 11));
        table.getTableHeader().setBorder(
                BorderFactory.createMatteBorder(0, 0, 1, 0, new Color(200, 220, 255)));
        ...
        scroll.setBorder(BorderFactory.createLineBorder(new Color(200, 220, 255), 1));
    ...
    private JPanel buildQuickActions() {
        ...
        card.add(makeActionBtn("🏥  New Encounter",        SKY,   SKY_SOFT));
        card.add(Box.createVerticalStrut(10));
        card.add(makeActionBtn("💊  Issue Prescription",   LILAC, LILAC_SOFT));
        card.add(Box.createVerticalStrut(10));
        card.add(makeActionBtn("🧪  Create Lab Order",     MINT,  MINT_SOFT));
        card.add(Box.createVerticalStrut(10));
        card.add(makeActionBtn("📋  View Prescriptions",   PEACH, PEACH_SOFT));
        card.add(Box.createVerticalStrut(10));
        card.add(makeActionBtn("👤  Patient History",      PINK_HOT, PINK_PALE));
        card.add(Box.createVerticalStrut(10));
        card.add(makeActionBtn("🔐  Manage Consent",       new Color(150, 200, 150),
    ...
    private void loadStats() {
        lblPatientsSeen.setText("...");
        lblPrescriptions.setText("...");
        lblLabOrders.setText("...");
        lblEncountersToday.setText("...");
        ...
        new SwingWorker<int[], Void>() {
            ...
            @Override protected int[] doInBackground() {
                try (Connection conn = DBConnection.getConnection()) {
                    ...
                    // Stat 4: Encounters today
                    try (PreparedStatement ps = conn.prepareStatement("""
                        SELECT COUNT(*)
                        FROM   encounter
                        WHERE  doctor_id = ?
                          AND  visit_datetime::date = CURRENT_DATE
                    """)) {
                        ps.setInt(1, userId);
                        ResultSet rs = ps.executeQuery();
            ...
            @Override protected void done() {
                try {
                    int[] c = get();
                    lblPatientsSeen.setText(String.valueOf(c[0]));
                    lblPrescriptions.setText(String.valueOf(c[1]));
                    lblLabOrders.setText(String.valueOf(c[2]));
                    lblEncountersToday.setText(String.valueOf(c[3]));
    ...
    private void loadRecentActivity(Connection conn) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement("""
            SELECT e.encounter_id,
                   e.patient_id,
                   COALESCE(e.diagnosis_summary, e.complaint_summary, 'Visit') AS detail,
                   e.visit_datetime::date AS dt
            FROM   encounter e
            WHERE  e.doctor_id = ?
            ORDER  BY e.visit_datetime DESC
            LIMIT  3
        """)) {
            ps.setInt(1, userId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ...
                if (detail != null && detail.length() > 30)
                    detail = detail.substring(0, 27) + "...";
                final String[] row = {
                        "🏥 Encounter",
                        "Patient #" + rs.getInt("patient_id") + " — " + detail,
                        rs.getString("dt"),
                        "Done"
                };
        ...
        try (PreparedStatement ps = conn.prepareStatement("""
            SELECT prescription_id, patient_id, issue_date, status
            FROM   prescription
            WHERE  doctor_id = ?
            ...
            while (rs.next()) {
                ...
                final String[] row = {
                        ...
                        "Rx #" + rs.getInt("prescription_id") +
                                " for Patient #" + rs.getInt("patient_id"),
        ...
        // Recent lab orders
        try (PreparedStatement ps = conn.prepareStatement("""
            SELECT lab_order_id, patient_id, order_date, status
            FROM   lab_order
            WHERE  doctor_id = ?
    ...
    public static void main(String[] args) {
        ...
        LoginScreen.loggedInUserId = 10;
        LoginScreen.loggedInEmail  = "dr.smith@example.com";
        LoginScreen.loggedInRole   = "DOCTOR";

        SwingUtilities.invokeLater(() -> {
            JFrame f = new JFrame("Doctor Dashboard Test");
            f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            f.setSize(950, 660);
            f.setLocationRelativeTo(null);
            f.setContentPane(new DoctorDashboard());
            f.setVisible(true);
        });
    }
}
